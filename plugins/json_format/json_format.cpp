  ...
  class JsonFormatPlugin : public draconis::core::plugin::IOutputFormatPlugin {
   ...
   public:
    ...
    fn formatOutput(const String& formatName, const Map<String, String>& data) const -> Result<String> override {
      ...
      output.kernelVersion      = getOptional("kernel_version");
      output.operatingSystem    = getOptional("operating_system");
      output.osName             = getOptional("os_name");
      output.osVersion          = getOptional("os_version");
      output.osId               = getOptional("os_id");
      output.memInfo            = getOptional("memory_info");
      output.memUsedBytes       = getOptionalU64("memory_used_bytes");
      output.memTotalBytes      = getOptionalU64("memory_total_bytes");
      output.desktopEnv         = getOptional("desktop_environment");
      output.windowMgr          = getOptional("window_manager");
      output.diskUsage          = getOptional("disk_usage");
      ...
      output.cpuModel           = getOptional("cpu_model");
      output.cpuCoresPhysical   = getOptionalU32("cpu_cores_physical");
      output.cpuCoresLogical    = getOptionalU32("cpu_cores_logical");
      output.gpuModel           = getOptional("gpu_model");
      output.uptime             = getOptional("uptime");
      output.uptimeSeconds      = getOptionalI64("uptime_seconds");
      output.packageCount       = getOptionalU64("package_count");
      output.nowPlayingTitle    = getOptional("now_playing_title");
      output.nowPlayingArtist   = getOptional("now_playing_artist");
